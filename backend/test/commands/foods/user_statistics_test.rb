...
class Foods::UserStatisticsTest < BaseTest
  ...
  test 'success - no users' do
    # given
    User.delete_all
    options = { data_source: Food.all.to_a, limit: 10, offset: 0 }
  ...
  test 'success - one food' do
    ...
    options = { data_source: Food.all.to_a, limit: 10, offset: 1 }
  ...
  test 'success - complex' do
    ...
    assert(command.result)
    assert_equal(:ok, command.status)
    expected = [{ :user_id=>@user.id, :value=>0.0},
                { :user_id=>user1.id, :value=>"9000.25" },
                { :user_id=>user2.id, :value=>"16.0" },
                { :user_id=>user3.id, :value=>"3000.0" }].sort_by {|o| o[:user_id]}
